...
tasstart = True
tastime = 0
wrhook = open("token.txt").read().split("\n")[0]
tashook = open("token.txt").read().split("\n")[1]
while True:
    try:
            ...
            while not finished: # cycles through all runs until an already processed run is reached
                ...
                if run["id"] in checked_ids:
                    ...
                    if jsonget(toprun)["data"]["runs"][0]["run"]["id"] == run["id"]:
                        ...
                        r = requests.post(wrhook, json={"content": wr, "username": "WRBot"})
        ...
        if tasstart:
            ...
            while runsarenew:
                ...
                if newrun.guid not in taschecked:
                    ...
                    runtime = titlesearch.group(4)

                    runinfo = f"{game} {category} by {runner} in {runtime}\n<{newrun.link}>\n{videolink}"
                    r = requests.post(tashook, json={"content": runinfo, "username": "TASBot"})
